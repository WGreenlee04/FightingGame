	...
	public void run() {
		...
		while (running) {
			...
			for (int i = 0; i < space.getItems().size(); i++) {
				if (space.getItems().get(i).getY() + space.getItems().get(i).getCurrentImage().getHeight(space)
						+ 30 > space.getHEIGHT()) {
					space.getItems().get(i).setY(
							space.getHEIGHT() - (space.getItems().get(i).getCurrentImage().getHeight(space) + 30));
				}
			}
			running = false;
		}
